...
void setup()
{
  ...
  // Delay 3 seconds for smoke sensor to warm up
  delay(3000);
}

void loop()
{
  sensorValue = digitalRead(DIGITAL_PIN);

  // If sensore value is 0 (smoke/motion detected), turn on buzzer and LED
  if (!sensorValue)
  {
    tone(8, 523, 100);
    digitalWrite(3, HIGH);
...
unsigned long hexToUlong(String s)
{
  ...
  for (i = 7; i >= 0; i--)
  {                        // end to the start of the string
...
void processDataMessage()
{
  ...
  if (incCount > nodeCounter[from])
  {
    ...
    if (nodeTTLs[node] > 8)
    {                        // 8 hops or 1 full cycles time (~16 sec)
    ...
    if (node == myNode && from != myNode)
    { // if data is meant for me and not from me
      Serial.print("From: ");
      Serial.print(from);
      Serial.print(" To: ");
      Serial.print(node);
